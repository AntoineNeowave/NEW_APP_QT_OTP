rem PyInstaller main.py --name NeoOTP --onefile --noconsole --clean --add-data "ui;ui"  --add-data "images;images"
PyInstaller main.py --name NeoOTP --onefile --noconsole --clean --hidden-import fido2.hid --hidden-import fido2.pcsc --hidden-import smartcard --collect-submodules fido2 --collect-binaries hid --collect-binaries smartcard --collect-data smartcard --add-data "ui;ui"  --add-data "images;images" --icon=images/logo.ico --clean --log-level=DEBUG
rem ajouter --runtime-tmpdir pour extraire les DLL a côté de .exe , pour un lancement plus rapide
rem Ou bien utiliser nuitkla